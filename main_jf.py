...
class HuYa:
    ...
    def login(self, username, password):
        ...
        if not self.login_check():
            ...
            requests.get(
                'https://bark.s6.design/xd3sw7KLKLx4zpxMVQ8cZ6/虎牙登陆二维码扫码?url={}'.format(qr_url))
    ...
    def stopPlayer(self):
        ...
        try:
            self.driver.execute_script('''
                if(document.getElementsByClassName("player-pause-btn").length != 0){
                    document.getElementsByClassName("player-pause-btn")[0].click();
                }
            ''')
            logging.info("停止播放成功")
        except Exception as e:
            logging.error("停止播放失败: {}".format(e))
    ...
    def getTreasure(self, room_id):
        ...
        # 领取手机看任意直播宝箱
        logging.info("手机宝箱: 准备领取宝箱")
        taskList = self.driver.find_element(By.CLASS_NAME, "box-task-list")
        items = taskList.find_elements(By.CLASS_NAME, "task-item")
        for index, item in enumerate(items):
            logging.info("手机宝箱: 第{}个: {}".format(index + 1, item.text))
            try:
                button = item.find_element(By.CSS_SELECTOR, ".btn.finished")
                if button and button.text == "领取":
                    button.click()
                    logging.info("手机宝箱: 第{}个任务: {} 已经完成".format(index + 1, item.text))
                    time.sleep(5)
                    break
            except NoSuchElementException:
                logging.info("手机宝箱: 第{}个: 没有领取按钮，跳过".format(index + 1))
                continue
    ...
    def sendMsg(self, group, title, msg):
        ...
        print('https://bark.s6.design/xd3sw7KLKLx4zpxMVQ8cZ6/{}/{}?group={}&icon={}'.format(title, msg, group, icon))
        requests.get(
            'https://bark.s6.design/xd3sw7KLKLx4zpxMVQ8cZ6/{}/{}?group={}&icon={}'.format(title, msg, group, icon))
